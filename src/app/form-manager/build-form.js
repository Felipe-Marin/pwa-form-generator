...
gemerateFormsComponents(config.forms);
...
function generateFormsComponents(forms){
    ...
    forms.forEach(form => {
        ...
        fs.writeFileSync(destFormsFolder + `form-n${form.formNumber}.component.html`, htmlContent);
        fs.writeFileSync(destFormsFolder + `form-n${form.formNumber}.component.ts`, tsContent);
        fs.writeFileSync(destFormsFolder + `form-n${form.formNumber}.component.css`, cssContent);
    });
}
...
function generateFormsList(config){
    ...
    const destFormListFile = 'src/app/app-routing.module.ts'
